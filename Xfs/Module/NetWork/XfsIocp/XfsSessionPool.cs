...
namespace Xfs
{
    public class XfsSessionPool : XfsComponent
    {
        private ConcurrentQueue<XfsSession> queue;
        public int _maxSession = 10;

        public XfsSessionPool()
        {
            this.Init();
        }

        public XfsSessionPool(Int32 _maxSession)
        {
        ...
        private void Init()
        {
            ...
            for (int i = 0; i < this._maxSession; i++)
            {
                ...
                session.GetComponent<XfsHeartComponent>().Close();
                session.GetComponent<XfsAsyncUserToken>().Close();
                session.Close();
